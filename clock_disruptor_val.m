function dis_node_val = clock_disruptor_val(func, param, max, min, t)
    ...
    switch func
        case "sine_wave"
            dis_node_val = (avg+param(1,5)) + (param(1,3)*range)*sin( (param(1,2)*t)+param(1,4) ) + param(1,1)*t;
        case "sine_small_amp"
        ...
        case "noise_const"
            a = rand;
            if a>0.5
                dis_node_val = avg + param(1,1)*t + rand(1, t(end))*35;
            else 
                dis_node_val = avg + param(1,1)*t - rand(1, t(end))*25;
            end
    end
